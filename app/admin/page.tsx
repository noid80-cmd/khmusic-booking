...
export default function AdminPage() {
  ...
  async function deleteUser(accountId: string, authUserId: string, name: string) {
    ...
    // 서비스 롤 키 미설정 시 accounts 레코드만 삭제
                ...
                <div key={u.id} className="rounded-2xl bg-white"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => deleteUser(u.id, u.user_id, u.name)}
                        className="text-xs font-medium rounded-xl border"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flex: '1 1 0%', minWidth: 0, position: 'relative', overflow: 'hidden' }}>
                <input type="date" value={lockDate}
                  ...
                  style={{ width: '100%', minWidth: 0, padding: '14px 16px', colorScheme: 'light', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                {lockDate === todayStr() && (
                  <span style={{ position: 'absolute', top: '50%', right: 12, transform: 'translateY(-50%)', fontSize: 11, fontWeight: 700, color: '#6366f1', background: '#eef2ff', borderRadius: 8, padding: '2px 7px', pointerEvents: 'none' }}>오늘</span>
                )}
              </div>
